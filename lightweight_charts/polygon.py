...
async def async_get_bar_data(
    ticker: str, timeframe: str, start_date: str, end_date: str, limit: int = 5_000
):
    loop = asyncio.get_running_loop()
    return await loop.run_in_executor(
        None, get_bar_data, ticker, timeframe, start_date, end_date, limit
    )
...
async def subscribe(ticker: str, sec_type: SEC_TYPE, func, args, precision=2):
    ...
    if not _lasts.get(key):
        _lasts[key] = {"price": 0, "funcs": [], "precision": precision}
        if sec_type != "indices":
            _lasts[key]["volume"] = 0
...
async def _websocket_connect(sec_type):
    if websockets is None:
        raise ImportError(
            'The "websockets" library was not found, and must be installed to pull live data.'
        )
    ticker_key = {
        "stocks": "sym",
        "options": "sym",
...
class PolygonAPI:
    ...
    async def async_set(
        ...
        await subscribe(
            ticker,
            sec_type,
            self._chart.update_from_tick,
            (True,),
            self._chart.num_decimals,
        )
        return True
...
class PolygonChart(Chart):
    ...
    async def _polygon(self, symbol):
        ...
        start_date = (
            ...
            else dt.datetime.strptime(self.end_date, "%Y-%m-%d")
        )
