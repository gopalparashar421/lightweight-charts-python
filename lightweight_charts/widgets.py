...
class WxChart(abstract.AbstractChart):
    def __init__(
        ...
        super().__init__(
            abstract.Window(
                self.webview.RunScript, "window.wx_msg.postMessage.bind(window.wx_msg)"
            ),
            inner_width,
            inner_height,
            scale_candles_only,
...
class QtChart(abstract.AbstractChart):
    def __init__(
        ...
        self.webview.loadFinished.connect(
            lambda: self.webview.page().runJavaScript("""
...
class StaticLWC(abstract.AbstractChart):
    def __init__(
        ...
        with open(abstract.INDEX.replace("index.html", "styles.css")) as f:
            css = f.read()
